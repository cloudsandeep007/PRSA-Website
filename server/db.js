...
try {
    ...
    if (!fs.existsSync(dbPath)) {
      if (fs.existsSync(sourceDb)) {
        try {
          ...
          console.error('Failed to copy database file to /tmp:', copyErr);
        }
      }
    }
  ...
  console.error('Error initializing SQLite file database, falling back to in-memory:', err);
  try {
    db = new Database(':memory:');
  } catch (memErr) {
    console.error('Fatal SQLite memory database error:', memErr);
  }
}
