...
console.log("Seeding database with PRSA content...");
...
// Seed Programs
const countProg = db.prepare("SELECT COUNT(*) as count FROM programs").get().count;
if (countProg === 0) {
  const progs = [
    {
  ...
  console.log("Seeded 6 Skating Programs");
}

// Seed Coaches
const countCoach = db.prepare("SELECT COUNT(*) as count FROM coaches").get().count;
if (countCoach === 0) {
  const coaches = [
    {
...
// Seed Events
const countEvents = db.prepare("SELECT COUNT(*) as count FROM events").get().count;
if (countEvents === 0) {
  const evts = [
    {
...
// Seed Gallery
const countGal = db.prepare("SELECT COUNT(*) as count FROM gallery").get().count;
if (countGal === 0) {
  for (let i = 1; i <= 15; i++) {
  ...
  console.log("Seeded 15 Gallery Media Items");
}

// Seed Testimonials
const countTest = db.prepare("SELECT COUNT(*) as count FROM testimonials").get().count;
if (countTest === 0) {
  const tests = [
    {
      name: "Meera Kulkarni",
...
// Seed Locations
const countLoc = db.prepare("SELECT COUNT(*) as count FROM locations").get().count;
if (countLoc === 0) {
  const locs = [
    {
...
// Seed FAQs
const countFaq = db.prepare("SELECT COUNT(*) as count FROM faqs").get().count;
if (countFaq === 0) {
  const faqs = [
    {
